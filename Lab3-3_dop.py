# користувач вводить ймовірність мутації
import random
import kivy.app
...
class SimpleApp(kivy.app.App):
    def build(self):
        ...
        self.prob_mut_label = kivy.uix.label.Label()
        self.textInput_prob_mut = kivy.uix.textinput.TextInput(text="0.001")
        ...
        self.boxLayout.add_widget(self.prob_mut_label)
        self.boxLayout.add_widget(self.textInput_prob_mut)
        self.prob_mut_label.text = "Ймовірність мутації"
    ...
    def displayMessage(self, btn):
        try:
            ...
            g = GenesContainer(count=11, coefs=coefs, y=int(self.textInput2.text), mutation_probability=float(self.textInput_prob_mut.text))
            result = g.result
