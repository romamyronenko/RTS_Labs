# вивести кількість затрачених ітерацій
import time
import kivy.app
...
def learn(dots, p, speed=0.008, count_of_iter=1000, deadline=float('inf')):
    start_time = time.time()
    w1, w2 = 0, 0
    flag = False
    for iter_num in range(count_of_iter):
        for (i, j) in dots:
            delta = p - (i*w1 + j*w2)
            w1 = w1 + delta*i*speed
            w2 = w1 + delta*j*speed
            if time.time() - start_time > deadline*1000:
                flag = True
                break
            if abs((w1*i + w2*j) - p) < 0.01:
                flag = True
                break
        if flag:
            break   
    return w1, w2, iter_num
